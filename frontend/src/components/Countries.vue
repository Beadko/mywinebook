  ...
  methods: {
    ...
    handleCountryChange(value) {
      if (value === "add_country") {
        (this.dialog_visible = true), (this.selected.country = "");
      }
    },
    clearSelection() {
      (this.selected.country = ""), (this.dialog_visible = false);
    },
  },
};
</script>
...
<template>
  <div class="flex items-center gap-2 mb-4">
    ...
    <Select
      ...
      :options="[...store.countries, { id: 'add_country', name: '+ Add New' }]"
      optionLabel="name"
      optionValue="id"
      class="w-full md:w-[14rem]"
      @change="handleCountryChange($event.value)"
    />
  </div>
  <Dialog
    v-model:visible="dialog_visible"
    modal
    header="Add a new country"
    :style="{ width: '25rem' }"
  >
    <div class="flex items-center gap-2 mb-4">
